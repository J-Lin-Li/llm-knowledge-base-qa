...
def hallucination_fallback(state):
    original = state.get("generation", "")
    return {"generation": f"⚠️ 系统提示：该回答经多次核查仍检测到潜在幻觉，内容可能不准确，请谨慎参考。\n\n{original}"}
...
workflow.add_node("hallucination_fallback", hallucination_fallback) # 幻觉超出兜底
workflow.add_node("not_useful_fallback", not_useful_fallback)   #生成无用回复超出次数兜底
...
workflow.add_conditional_edges(
    ...
    grade_generation_v_documents_and_question,  # 生成质量评估函数
    {
        ...
        "max_retries": "hallucination_fallback",  # 幻觉重试超限，强制结束
        "transform_max_retries": "not_useful_fallback",  # 答案质量改写超限，强制结束
    },
)
